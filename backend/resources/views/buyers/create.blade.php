...
@section('header', 'Create New Buyer')
    ...
    <div class="max-w-2xl mx-auto">
        <form action="{{ route('buyers.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <x-input id="buyer_name" name="buyer_name" type="text" class="mt-1 block w-full" required autofocus
            ...
            <div>
                ...
                <textarea id="address" name="address" rows="3" class="mt-1 block w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>{{ old('address') }}</textarea>
                @error('address')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
